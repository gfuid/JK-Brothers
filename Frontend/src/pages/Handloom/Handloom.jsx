...
export default function Handloom() {
  ...
  const handloomProducts = products.filter(p => p.category === 'HANDLOOM');

  // Filter products by subcategory and search term
  const filteredProducts = handloomProducts.filter(product => {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="bg-primary text-white p-8 md:p-12 rounded-sm mb-12 relative overflow-hidden shadow-md flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="relative z-10 text-center md:text-left max-w-xl">
            <h1 className="text-3xl md:text-5xl font-serif font-black tracking-wide mb-3 uppercase">Handloom Collection</h1>
            <p className="text-xs md:text-sm text-gray-300 font-medium leading-relaxed">
              Timeless heritage craftsmanship directly from Panipat's premium handloom factories. Woven with organic fibers, rich color fastness, and unparalleled durability.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-9">
            ...
            <div className="flex justify-between items-center mb-6 text-xs text-gray-500 font-medium">
              <p>Showing <span className="text-primary font-bold">{filteredProducts.length}</span> Handloom Products</p>
              {searchTerm && (
                <button 
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-sm border border-gray-100 p-8">
                <p className="text-gray-400 text-sm font-semibold mb-4">No handloom products match your selection.</p>
                <button 
                  onClick={() => setSearchParams({})}
